...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        logger.info(f"Nuevo cliente suscrito al Stream SDR. Totales: {len(self.active_connections)}")

    def disconnect(self, websocket: WebSocket):
        self.active_connections.remove(websocket)
        logger.info(f"Cliente WS desconectado. Restantes: {len(self.active_connections)}")
